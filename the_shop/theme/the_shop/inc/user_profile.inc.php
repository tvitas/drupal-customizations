<?php
function the_shop_preprocess_user_profile(&$variables) {
	if (isset($variables['user_profile']['summary']['#access'])) {
		$variables['user_profile']['summary']['#access'] = FALSE;
	}
}

function the_shop_preprocess_user_login(&$variables) {
...
function the_shop_preprocess_user_pass(&$variables) {
	if (!isset($variables['form']['#title'])) {
		$variables['form']['#title']['#markup'] = _t('_fogot_password_', t('Forgot password?'));
	}
}

function the_shop_preprocess_user_register(&$variables) {
	if (!isset($variables['form']['#title'])) {
		$variables['form']['#title']['#markup'] = _t('_register_account_', t('New account'));
	}
}

function the_shop_preprocess_user_pass_reset(&$variables) {
	if (!isset($variables['form']['#title'])) {
		$variables['form']['#title']['#markup'] = _t('_reset_password_', t('Reset password'));
	}
}

function the_shop_preprocess_user_profile_edit(&$variables) {
	if (!isset($variables['form']['#title'])) {
