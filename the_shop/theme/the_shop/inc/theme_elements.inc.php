<?php
function the_shop_container($variables) {
...
function the_shop_form_element($variables) {
  $element = &$variables['element'];
  ...
  if ($element['#type'] == 'textfield' || $element['#type'] == 'password') {
	...
	if (isset($element['#title']) && $element['#title']) {
		...
		$element['#children'] = substr_replace($element['#children'], $placeholder, 7, 0);
	}
  ...
  if ($element['#type'] == 'textarea') {
	...
	if (isset($element['#description'])) {
		$tran = trim(_t(_gid(strip_tags($element['#title']) . '_desc'), t(strip_tags($element['#description']))));
		$tooltip = ' title="' . $tran . '" ';
		$element['#children'] = substr_replace($element['#children'], $tooltip, 54, 0);
		unset($element['#description']);
	}
  }
...
//    $output .= '<div class="description">' . t($element['#description']) . "</div>\n";
    $output .= '<p class="uk-form-help-block uk-text-muted"><small>' . $element['#description'] . "</small></p>\n";
  }
...
function the_shop_textarea($variables) {
  $element = $variables['element'];
...
function the_shop_item_list($variables) {
...
function the_shop_form_required_marker($variables) {
  ...
  if (module_exists('strings_by_id')) {
...
function the_shop_form_element_label($variables) {
  $element = $variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
 ...
 */
function the_shop_button($variables) {
  $element = $variables['element'];
  $label = $element['#value'];
...
function the_shop_preprocess_button(&$variables) {
  ...
  if (isset($variables['element']['#value'])) {
    if ($class = _the_shop_colorize_button($variables['element']['#value'])) {
